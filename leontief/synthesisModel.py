...
# Основные рассчеты
def synCalc(A, X, Y, Yj, E, n, W, AXY):
    # Рассчет обратной матрицы E-A
    oldC = E - A
    c = np.array(np.matrix(oldC).I)
...
def synOutput(root, startA, startW, startX, startYj, startAXY, Y, A, n, aText):
    ...
    for i in range(n + 12, n + n + 12):
        mes = StringVar(root, round(Y[i - n - 12], 3))
        wEntry = Entry(root, textvariable=mes)
...
def synProd(root, A, W, Y, Yj, AXY, X, n, prodBtn):
...
def synModel(root, A, Y, n):
    ...
    while 1:
        ...
        if myCheck == 0:
            ...
            aText = []
            synOutput(root, startA, startW, startX, startYj, startAXY, Y, A, n, aText)
            ...
            if aText[0] != 'Таблица продуктивна.':
                ...
                prodBtn = Button(text='Улучшить\n продуктивность',
                                 command=lambda:synProd(root, A, W, Y, Yj, AXY, X, n, prodBtn))
